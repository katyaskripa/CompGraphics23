...
TEST_CASE( "Intersection does not exist", "[single-file]" )
{
    ...
    obj::Disk disk{ 1.0f,
                    lmath::Vec3{ 0.0f, 0.0f, -2.0f }.normalize(),
                    lmath::Point3{ 0.0f, 1.0f, 1.0f } };

    REQUIRE( disk.hit( ray, 0.001f, 100.0f, hit ) == false );
}
...
TEST_CASE( "Intersection exists", "[single-file]" )
{
    ...
    render::HitRecord hit;
    obj::Disk disk{ 5.0f,
                    lmath::Vec3{ 0.0f, 1.0f, 1.0f }.normalize(),
                    lmath::Point3{ 0.0f, 1.0f, -2.0f } };

    REQUIRE( disk.hit( ray, 0.001f, 100.0f, hit ) == true );
}
